...
@implementation FLDuanYuSliderBar
...
- (void)switchSliderBar:(NSInteger)X {
    [self touchTopSliderBar:self.btnArray[X]];
}
...
- (void)reloadFont {
    for (UIButton *btn in self.btnArray) {
        [btn.titleLabel reloadFont];
    }
}

#pragma mark - private methods
- (void)setupUI {
    self.backgroundColor = RGB(251, 115, 110);
...
- (void)setupSliderbarItems:(NSArray<NSString *> *)titleSource {
    ...
    for (NSInteger i = 0; i < titleSource.count; i ++) {
        ...
        [item setTitleColor:(item.selected ? [UIColor whiteColor] : RGB(31, 31, 31)) forState:UIControlStateNormal];
        ...
        item.frame = CGRectMake((itemW + distance) * i + distance, 10, itemW, itemH);
